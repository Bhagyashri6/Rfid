...
import static com.enpeck.RFID.DailyReport.DailyReportFragment.listReport;
 ...
 * @author Brais Gabín
 */
public abstract class SampleTableAdapterDaily extends BaseTableAdapter {
     ...
     *            The current context.
     */
    public SampleTableAdapterDaily(Context context) {
                   ...
                   /* Toast.makeText(getContext(),"NO date is added",Toast.LENGTH_LONG).show();*/
                    ...
                    try {
                        if (row == -1) {
                            ...
                            Intent intent = new Intent("call.DailyReport2.action");
                            ...
                            intent.putExtra("Contno", listReport.get(row).getContainerno());
                            intent.putExtra("serialno", listReport.get(row).getSerialno());
                            intent.putExtra("ieccode", listReport.get(row).getIECno());
                            intent.putExtra("eseal", listReport.get(row).getE_sealno());
                            intent.putExtra("sealingdate", listReport.get(row).getS_billDate());
                            intent.putExtra("sealingtime",listReport.get(row).getS_billtime());
                            getContext().sendBroadcast(intent);
